...
impl AStar {
    ...
    fn construct_path(
        history: &HashMap<Position, Position>,
        current: Position,
        start: Position,
    ) -> Vec<Position> {
    ...
    pub fn search(
        ...
        }) = frontier.pop()
        {
            if current_position == goal {
                return Some(AStar::construct_path(&came_from, current_position, start));
            }
            ...
            for (neighbour_position, neigbour_cost) in map.get_valid_neighbours(current_position) {
                ...
                if should_consider_neighbour {
                    ...
                    f_score.insert(
                        neighbour_position,
                        tentative_score + heuristic(neighbour_position, goal),
                    );
                    ...
                    if !frontier.iter().any(|e| e.position == neighbour_position) {
                        frontier.push(State {
                            ...
                            cost: f_score[&neighbour_position],
                        });
                    }
                }
            }
...
pub fn simple(current_position: Position, goal: Position) -> i32 {
...
mod tests {
    ...
    use crate::{map::Map, pos::Position, print_map, simple, AStar};
