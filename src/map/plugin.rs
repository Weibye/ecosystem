...
pub(crate) fn generate_map(settings: &MapSettings, rng: &mut GlobalRng) -> Map {
    ...
    for x in 0..settings.width {
        for y in 0..settings.height {
            ...
            let noise_value = noise.get([x as f64 * SCALE / settings.width as f64, y as f64 * SCALE / settings.height as f64]);
            tiles.push(
                if noise_value > 0.9 { TileType::Rock } 
                else if noise_value > 0.2 { TileType::Grass }
                else if noise_value > 0.0 { TileType::Sand }
                else if noise_value > -0.3 { TileType::ShallowWater }
                else { TileType::DeepWater }
            );
        }
    }
